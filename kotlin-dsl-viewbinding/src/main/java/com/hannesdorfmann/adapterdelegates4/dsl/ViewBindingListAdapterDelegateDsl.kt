...
inline fun <reified I : T, T, V : ViewBinding> adapterDelegateViewBinding(
	...
	return CachingViewBindingAdapterDelegate(
			binding = viewBinding,
			on = on,
...
open class CachingViewBindingAdapterDelegate<I : T, T, V : ViewBinding>(
	...
	private val boundViewHolders = HashSet<AdapterDelegateViewBindingViewHolder<I, V>>()
	fun boundViewHolders(): Set<AdapterDelegateViewBindingViewHolder<I, V>> = Collections.unmodifiableSet(boundViewHolders)
	...
	override fun onCreateViewHolder(parent: ViewGroup): AdapterDelegateViewBindingViewHolder<I, V> {
		...
		return AdapterDelegateViewBindingViewHolder<I, V>(
				this::class.qualifiedName, binding
		).also {
			initializerBlock(it)
		}
	}
	...
	open fun onRVDetachedFromWindow() {
...
class AdapterDelegateViewBindingViewHolder<T, V: ViewBinding>(
		val tag: String?, val binding: V, view: View = binding.root
